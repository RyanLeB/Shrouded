...
func tween_collect(percent: float, start_position: Vector2):
	...
	var target_rotation_degrees = direction_from_start.angle() + deg_to_rad(90)
	rotation_degrees = lerp_angle(rotation, target_rotation_degrees, 1 - exp(-2 * get_process_delta_time()))
